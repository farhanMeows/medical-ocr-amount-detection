...
const normalizeAmounts = (rawTokens, requestId) => {
  try {
    logger.info('Starting normalization', { requestId, tokenCount: rawTokens.length });
    ...
    const confidence = totalAttempts > 0 
      ? successfulNormalizations / totalAttempts 
      : 0;
...
const normalizeToken = (token) => {
  ...
  for (const [wrong, correct] of Object.entries(charReplacements)) {
    ...
    const regex = new RegExp(`(\\d)${wrong}(?=\\d)|(?<=\\d)${wrong}(?=\\d)|^${wrong}(?=\\d)|(?<=\\d)${wrong}$`, 'g');
...
const validateNormalizedAmounts = (amounts) => {
  ...
  const hasUnreasonableAmount = amounts.some(amt => amt > maxReasonableAmount);
